...
def main():
    ...
    with mp_hands.Hands(
        ...
        # mqtt_timer = time.time()  # display timer
        ...
        while stream_video.isOpened():
            ...
            game.gest_update(gest=gest_class)
            for card in curr_cards:
                game.game_update(card=card)
            ...
            # if (time.time() - mqtt_time) > 1.0:
            #     send_msg_by_mqtt(
            #         pi_ip, pi_channel, f"msg: {msg}"
            #     )
            #     mqtt_time = time.time()
